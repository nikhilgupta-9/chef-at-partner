(function () {
  ...
  nv.models.tooltip = function () {
    ...
    var id = "nvtooltip-" + Math.floor(Math.random() * 100000),
      ...
      duration = 100,
      headerEnabled = true,
      nvPointerEventsClass = "nv-pointer-events-none";
  ...
  nv.utils.inheritOptionsD3 = function (target, d3_source, oplist) {
    ...
    target._d3options = (target._d3options || []).filter(function (
      item,
      i,
      ar
    ) {
        ...
        if (
          ...
          wrap.selectAll("g.nv-axisMaxMin").each(function (d, i) {
            try {
              ...
              if (i) maxMinRange.push(scale(d) - 4);
              else maxMinRange.push(scale(d) + 4);
            }
          });
          g.selectAll("g").each(function (d, i) {
            if (scale(d) < maxMinRange[0] || scale(d) > maxMinRange[1]) {
              if (d > 1e-10 || d < -1e-10) d3.select(this).remove();
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        if (showYAxis) {
          yAxis
            .scale(y)
            .ticks(Math.floor(availableHeight / 36))
  ...
  nv.models.bulletChart = function () {
    ...
    bullet.dispatch.on("elementMouseover.tooltip", function (evt) {
      evt["series"] = { key: evt.label, value: evt.value, color: evt.color };
      tooltip.data(evt).hidden(false);
    });
  ...
  nv.models.distroPlot = function () {
    ...
    function select_sigma(x) {
      var sorted = x.sort(d3.ascending);
      var normalize = 1.349;
      var IQR =
        (d3.quantile(sorted, 0.75) - d3.quantile(sorted, 0.25)) / normalize;
    ...
    function prepData(dat) {
      function calcStats(g, xGroup) {
        var v = g
          .map(function (d) {
            if (colorGroup) allColorGroups.add(colorGroup(d));
            return getY(d);
          })
          .sort(d3.ascending);
        ...
        if (typeof d3.beeswarm !== "undefined") {
          ...
          observations.map(function (e, i) {
            ...
            e.isOutlier = e.datum < wl.iqr || e.datum > wu.iqr;
            e.isOutlierStdDev = e.datum < wl.stddev || e.datum > wu.stddev;
            e.randX =
              Math.random() *
              jitter *
    ...
    function makeNotchBox(boxLeft, notchLeft, boxCenter, dat) {
      var boxPoints;
      var y = centralTendency == "mean" ? getMean(dat) : getQ2(dat);
      if (notchBox) {
        boxPoints = [
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        yScale
          ...
          .nice()
          .range(yRange || [availableHeight, 0]);
        if (typeof reformatDat === "undefined") reformatDat = prepData(data);
        ...
        distroplots
          ...
          .on("mouseover", function (d, i, j) {
            ...
            if (showOnlyOutliers && plotType == "box" && !isOutlier(d)) return;
            var fillColor = d3.select(this.parentNode).style("fill");
  ...
  nv.models.distroPlotChart = function () {
    ...
    function dataHasChanged(d) {
      if (arraysEqual(d, dataCache)) {
        ...
        dataCache = JSON.parse(JSON.stringify(d));
        return true;
      }
    }
  ...
  nv.models.heatMap = function () {
      ...
      if (normTypes.indexOf(normalize) != -1) {
        ...
        dat.forEach(function (cell, i) {
          if (cellsAreNumeric()) {
            ...
            cell._cellPos.norm = getCellValue(cell);
          }
        });
      } else {
        normalize = false;
    ...
    function prepData(data) {
      ...
      data.forEach(function (cell) {
        ...
        cell._cellPos = { idx: idx, ix: uniqueX[valX], iy: uniqueY[valY] };
        idx++;
        combo = [valX, valY];
      ...
      Object.keys(uniqueY).forEach(function (j) {
        Object.keys(uniqueX).forEach(function (i) {
          ...
          if (cellVal) {
            ...
            var cellPos = { idx: idx, ix: uniqueX[i], iy: uniqueY[j] };
            idx++;
            reformatData.push({ _cellPos: cellPos });
          }
        });
      });
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        allMetaRect
          .on("mouseover", function (d, i) {
            var isColMeta =
              d3.select(this).attr("class").indexOf("x-meta") != -1
                ? true
                : false;
            d3.selectAll(".nv-cell").each(function (e) {
  ...
  nv.models.heatMapChart = function () {
    ...
    function chart(selection) {
      ...
      d3.selectAll(".nv-axis").selectAll("line").style("stroke-opacity", 0);
      ...
      selection.each(function (data) {
        ...
        interactiveLayer.dispatch.on("elementMousemove", function (e) {
          ...
          interactiveLayer.tooltip
            ...
            .data({ value: xValue, index: pointIndex, series: allData })();
          interactiveLayer.renderGuideLine(pointXLocation);
        });
        interactiveLayer.dispatch.on("elementMouseout", function (e) {
  ...
  nv.models.linePlusBarChart = function () {
    ...
    bars.dispatch.on("elementMouseover.tooltip", function (evt) {
      ...
      evt["series"] = { value: chart.y()(evt.data), color: evt.color };
      tooltip
        .duration(0)
        .valueFormatter(function (d, i) {
          ...
          hideable = [
            {
              values: data[0].values.map(function (d) {
                return { x: d.x, y: 0, series: d.series, size: 0.01 };
              }),
            },
          ];
        if (stacked) {
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        foreground.on("mouseout", function (d, i) {
          ...
          dispatch.elementMouseout({ label: d.name, index: i });
        });
        ...
        function brush() {
          ...
          updateTicks();
          dispatch.brush({ filters: filters, active: active });
        }
        function brushend() {
  ...
  nv.models.parallelCoordinatesChart = function () {
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        parallelCoordinates.dispatch.on(
          "brushEnd",
          function (active, hasActiveBrush) {
            if (hasActiveBrush) {
              displayBrush = true;
    ...
    parallelCoordinates.dispatch.on("elementMouseover.tooltip", function (evt) {
      var tp = { key: evt.label, color: evt.color, series: [] };
      if (evt.values) {
  ...
  nv.models.scatter = function () {
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        var logScale = typeof chart.yScale().base === "function";
        var seriesData =
      ...
      selection.each(function (data) {
        ...
        interactiveLayer.dispatch.on("elementMousemove", function (e) {
          ...
          if (
            ...
            allData.push({ key: totalLabel, value: valueSum, total: true });
          }
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        cGE
          ...
          .on("click", function (d, i) {
            zoomClick(d);
            dispatch.elementClick({ data: d, index: i });
          })
          .on("mouseover", function (d, i) {
